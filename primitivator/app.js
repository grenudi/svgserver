...
let jsdom = new JSDOM(fs.readFileSync(join(__dirname,"./mock.html"),"utf8"));
...
function convert (path) {
	...
	let file = new File({
		path: "./primitivator/pending/"+path,
		jsdom: true
	});
	...
	function onSubmit() {//e
		...
		// }
		let cfg = getConfig();
		Canvas.original(join(__dirname,"./pending/Tulips.jpg"), cfg).then(original => go(original, cfg));
	}
